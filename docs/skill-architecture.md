...
`scripts/discover-upstream-updates.py` observes exactly one selected provider
against its immutable pin and writes into the existing maintenance audit/receipt
lane. A newer upstream revision may prepare a scoped evidence packet, but
discovery never changes `upstreams.lock.json`, the active runtime, a branch, or
a pull request. Private or oversized release evidence blocks rather than being
silently truncated or rerouted.
