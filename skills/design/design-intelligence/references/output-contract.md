...
Recommended owner-local run folder:

```text
$STACK_DESIGN_STATE_ROOT/runs/YYYY-MM-DD/
```
...
```text
$STACK_DESIGN_STATE_ROOT/backfill/YYYY-MM-DD..YYYY-MM-DD/YYYY-MM-DD..YYYY-MM-DD/
```

Default `STACK_DESIGN_STATE_ROOT` to
`$HOME/.local/state/stack/design-intelligence`; never write generated packets
into the public repository or a runtime skill directory.
